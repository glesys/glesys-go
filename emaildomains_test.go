...
func TestEmailsList(t *testing.T) {
	c := &mockClient{body: `{"response":{"list":{"emailaccounts":[{"emailaccount":"user@example.com","displayname":"user@example.com","quotaingib": 2,"antispamlevel":3,"antivirus":"yes","autorespond":"yes","autorespondmessage":"This is not the account you are looking for.\n\nMove along, move along.","autorespondsaveemail":"yes","rejectspam":"no","created":"2019-10-26T13:07:13+02:00","modified":"2019-10-26T15:38:51+02:00"}],"emailaliases":[{"emailalias":"alias@example.com","displayname":"alias@example.com","goto":"user@example.com"}]}}}`}
	s := EmailDomainService{client: c}
	...
	assert.Equal(t, 2, (*emailaccounts)[0].QuotaInGiB, "quotaingib is correct")
...
func TestEmailEditAccount(t *testing.T) {
	c := &mockClient{body: `{"response":{"emailaccount":{"emailaccount":"user@example.com","displayname":"user@example.com","quotaingib": 20,"antispamlevel":3,"antivirus":"yes","autorespond":"yes","autorespondmessage":"This is not the account you are looking for.\n\nMove along, move along.","autorespondsaveemail":"yes","rejectspam":"no","created":"2019-10-26T13:07:13+02:00","modified":"2019-11-10T22:09:14+01:00"}}}`}
	s := EmailDomainService{client: c}

	editaccount, _ := s.EditAccount(context.Background(), "user@example.com", EditAccountParams{QuotaInGiB: 20})
	...
	assert.Equal(t, 20, editaccount.QuotaInGiB, "quotaingib is correct")
...
func TestCreateAccount(t *testing.T) {
	c := &mockClient{body: `{"response":{"emailaccount":{"emailaccount":"new_user@example.com","displayname":"new_user@example.com","quotaingib": 1,"antispamlevel":3,"antivirus":"yes","autorespond":"no","autorespondmessage":null,"autorespondsaveemail":"yes","rejectspam":"no","created":"2019-11-29T21:31:28+01:00","modified":null}}}`}
	...
	assert.Equal(t, 1, createaccount.QuotaInGiB, "quotaingib is correct")
...
func TestQuota(t *testing.T) {
	c := &mockClient{body: `{"response":{"quota":{"emailaccount":"user@example.com","usedquotainmib": 10,"quotaingib": 1}}}`}
	...
	assert.Equal(t, 10, quota.UsedInMiB, "used amount is correct")
	assert.Equal(t, 1, quota.QuotaInGiB, "quota in GiB is correct")
}
...
func TestCosts(t *testing.T) {
	...
	assert.Equal(t, 10.00, costs.Costs.Quota.Amount, "costs.quota.amount is correct")
	assert.Equal(t, 0.00, costs.Costs.Quota.Cost.Amount, "costs.quota.cost.amount is correct")
	assert.Equal(t, "SEK", costs.Costs.Quota.Cost.Currency, "costs.quota.cost.currency is correct")
	assert.Equal(t, 3.00, costs.Costs.Accounts.Amount, "costs.accounts.amount is correct")
	assert.Equal(t, 0.00, costs.Costs.Accounts.Cost.Amount, "costs.accounts.cost.amount is correct")
	...
	assert.Equal(t, 10.00, costs.PriceList.Quota.FreeAmount, "pricelist.quota.freeamount is correct")
	assert.Equal(t, 50.00, costs.PriceList.Accounts.Amount, "pricelist.accounts.amount is correct")
