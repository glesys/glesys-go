...
func TestRequestHasCorrectHeaders(t *testing.T) {
	...
	assert.Equal(t, "test-application/0.0.1 glesys-go/7.0.0", request.Header.Get("User-Agent"), "header User-Agent is correct")
...
func TestEmptyUserAgent(t *testing.T) {
	...
	request, err := client.newRequest(context.Background(), "GET", "/", nil)
	assert.NoError(t, err)
	assert.Equal(t, "glesys-go/7.0.0", request.Header.Get("User-Agent"), "header User-Agent is correct")
}
